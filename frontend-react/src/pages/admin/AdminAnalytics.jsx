  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-10">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white dark:bg-gray-800 p-6 rounded-[2rem] shadow-sm border border-gray-100 dark:border-gray-700">
        <div>
          <h1 className="text-3xl font-extrabold text-gray-900 dark:text-white tracking-tight">Detailed Analytics</h1>
          <p className="text-gray-500 dark:text-gray-400 font-medium mt-1">Deep dive into usage, performance, and outcomes.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <StatCard title="Daily Active (DAU)" value={demographics.dau.toLocaleString()} subtitle="Logged in today" icon={Activity} colorClass="bg-blue-500" />
        <StatCard title="Monthly Active (MAU)" value={demographics.mau.toLocaleString()} subtitle="Active last 30 days" icon={Users} colorClass="bg-indigo-500" />
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        <StatCard title="Sleep Goals" value={`${health_outcomes.sleep_completion}%`} subtitle="Users completing" icon={SmilePlus} colorClass="bg-indigo-500" />
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <StatCard title="Medical Records" value={feature_adoption.medical_records.toLocaleString()} subtitle="Files uploaded" icon={FileText} colorClass="bg-blue-500" />
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <StatCard title="API Error Rate" value={`${security.api_errors}%`} subtitle="Estimated failure rate" icon={XCircle} colorClass="bg-red-500" />
      </div>
