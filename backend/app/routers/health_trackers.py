...
async def process_voice_log(data: VoiceLogRequest, user_id: CurrentUserId, db: AsyncSession = Depends(get_db)):
    ...
    if intent.get("type") == "action":
        if intent.get("target_feature") == "jarvis_ai":
            action = intent.get("action_name")
            prompt = ""
            if action == "first_aid":
                query = intent.get("data", {}).get("query", "")
                prompt = f"The user is asking for first aid: '{query}'. Provide a very concise, step-by-step spoken response (max 4 sentences) to guide them. Do not use markdown or complex formatting."
            elif action == "triage":
                symptoms = intent.get("data", {}).get("symptoms", "")
                prompt = f"The user reports symptoms: '{symptoms}'. Provide a quick, empathetic spoken triage response advising whether to rest, medicate, or seek urgent care. Keep it under 3 sentences."
            ...
            if prompt:
                ...
                try:
                    response = await client.chat.completions.create(
                        ...
                        messages=[
                            {"role": "system", "content": "You are Jarvis, a concise and calm health assistant voice. Provide clear, spoken instructions."},
                    ...
                    return {
                        ...
                        "message": "Providing voice assistance..."
                    }
...
async def get_health_data(user_id: CurrentUserId, db: AsyncSession = Depends(get_db)):
    ...
    sleep_result = await db.execute(
        select(SleepEntry).where(SleepEntry.user_id == user_id).order_by(SleepEntry.date)
    )
