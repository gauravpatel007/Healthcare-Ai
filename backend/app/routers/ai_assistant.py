...
from app.schemas.chat import ChatMessageCreate, ChatMessageResponse, ChatResponse, ChatFeedbackRequest
...
async def chat(data: ChatMessageCreate, user_id: CurrentUserId, db: AsyncSession = Depends(get_db)):
    ...
    return ChatResponse(message_id=assistant_msg.id, response=response)
