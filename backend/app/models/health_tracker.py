...
class HealthEntry(Base, TimestampMixin):
    ...
    category: Mapped[str] = mapped_column(
        SAEnum("blood_sugar", "blood_pressure", "cholesterol", "weight",
               "heart_rate", "steps", "calories", "mindfulness",
...
class WaterLog(Base, TimestampMixin):
    ...
    date: Mapped[date] = mapped_column(Date, nullable=False)
    goal: Mapped[float] = mapped_column(Float, nullable=False, default=2000) # in ml
    completed: Mapped[float] = mapped_column(Float, nullable=False, default=0) # in ml
